...
%         load(fullfile(F(f).folder,fichier));
        ...
        stroke_pcoh5=nan(2,448,448);
        stroke_Pcov5=nan(2,896,896);
        stroke_Cov5=nan(2,896,896);


        for Hz=5:1:length(bandlabels)
            display(Hz);
...
%             stroke_Cov = cov(sourceDataReal);
            ...
            % debug: check nan
            sum(isnan(dataCovs_op),'all')
            % imagesc(squeeze(dataCovs_op(2,:,:)));colorbar
            ...
            % debug: check nan
            sum(isnan(stroke_Cov),'all')
            stroke_Cov5(Hz,:,:)=stroke_Cov;
            ...
            % debug: check nan
            sum(isnan(stroke_Pcov),'all')
            stroke_Pcov5(Hz,:,:)=stroke_Pcov;
           ...
           % addpath /home/zhibinz2/Documents/GitHub/AdaptiveGraphicalLassoforParCoh/Simulations/util
            ...
            % convert real value covariance to complex matrix (448x448)
            % then compute coherence
            stroke_pcoh=normalizeCSD(r2c(stroke_Pcov)); % ordinary coherence

            % debug: check nan
            sum(isnan(stroke_pcoh),'all')
            display(num2str(Hz))
            stroke_pcoh5(Hz,:,:)=stroke_pcoh;

%             if sum(sum(isnan(stroke_Pcov))) > 1
%                 stroke_Pcoh= NaN(448, 448);
%                 ii=ii+1
%                 BUG(ii,1)=d;
%                 BUG(ii,2)=f;
%                 BUG(ii,3)=Hz;
%                 disp('bug pour une bande de frequence avec histoire de NaN');
%             else
%             stroke_Pcoh=normalizeCSD(r2c(logical(stroke_Pcov)));% partial coherence using boolean
%             end
%             % SAVE
%           
%             Coh.(dossiers{d}).(bandlabels{Hz}).AGL.Coh(:,:,f)=stroke_coh;
%             Coh.(dossiers{d}).(bandlabels{Hz}).AGL.PCoh(:,:,f)=stroke_Pcoh;
% 
%             clearvars -except ii BUG Coh DOSSIERS dossiers Fs srnew downsample passbands bandlabels attenuation filtered_data Individual_Connectome filt_ds d dossier1 F f fichier example_data Hz

        end
%     end
% end

% save('Coherence_Patient.mat','Coh')% reconstruire BUG pour patient file


save('output_debug.mat','stroke_Pcov5',"stroke_pcoh5",'stroke_Cov5'); % reconstruire BUG pour patient file
